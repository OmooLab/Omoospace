...
class DirectoryTree(Tree):
    def __init__(self, structure: Structure = None, search_dir: str = None):
        self.root_path = None
        super().__init__()
        if structure:
    ...
    def from_structure(self, structure: Structure, parent_dir: str = None):
        if parent_dir:
    ...
    def add_dir(self, dir: str):
        ...
        for directory_path in directory_paths:
            ...
            directory_node = self.find(match=lambda node: node.path == node_path)
            if not directory_node:
                is_subspace = is_entity(Path(self.root_path, directory_path))
                directory = Directory(path=directory_path, is_subspace=is_subspace)
                directory_node = node.add(directory)
            node = directory_node
        return node

    @classmethod
    def get_dirs(cls, search_dir: str, recursive: bool = True) -> list[Path]:
    ...
    def make_dirs(self, root_dir: str = None):
        if not self.root_path and not root_dir:
            raise NotFoundError("root directory")
        self.__make_dirs(path=Path(root_dir) or self.root_path, node=self)

    @classmethod
    def __make_dirs(
        cls, path: Path, node: Union[Tree, Node], is_subspace: bool = False
    ):
    ...
    def __from_structure(
        ...
        for key in structure.keys():
            ...
            directory_node = self.find(match=lambda node: node.path == node_path)
            if not directory_node:
                is_subspace = key[0] == "*" or "Void" in directory_name.split("_")
                directory = Directory(path=directory_path, is_subspace=is_subspace)
                directory_node = node.add(directory)
