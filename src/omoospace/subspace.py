...
from omoospace.directory import Entity, Route
from omoospace.common import ProfileContainer
...
def get_route_entities(entity_path: str) -> list:
    ...
    for entity in entities:
        ...
        # e.g. `Seq010_Shot0100.blend` has two namespace: `Seq010` and `Shot0100`
        node_names: list[str] = entity_name.split("_")

        # clip matched route names as mush as possible.
        # e.g. route: ['Seq010'], enity name: `Seq010_Shot0100.blend`
        ...
        for node_name in node_names:
            route_entities.append({"node_name": node_name, "entities": [entity]})
    ...
    if "Void" in [d.get("node_name") for d in route_entities]:
        route_entities = [d for d in route_entities if d.get("node_name") != "Void"]
        ...
        route_entities.insert(0, {"node_name": "Void", "entities": void_entities})
