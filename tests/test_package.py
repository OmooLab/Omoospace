...
def test_import_package(mini_omoos_path: Path):
    ...
    write_file("Heart.glb", "Liver.glb", root_dir=Path(pkg_path, "Contents/Models"))
    write_file_content(
        (
            "Package.yml",
            """
    ...
    omoos.import_package(pkg_path, reveal_in_explorer=False)

    assert Path(omoos.contents_path, "Packages", "Organs").is_dir()
